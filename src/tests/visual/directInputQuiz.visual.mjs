...
const { check, runCase, finish } = createSuite('Phase 1-C2 直接入力テスト');
...
try {
  ...
  for (const viewport of KEYBOARD_VIEWPORTS) {
    ...
    await runCase(label, async () => {
      ...
      try {
        ...
        return ` 出題・入力欄・こたえる がすべて可視（入力欄 ${input.height.toFixed(0)}px / ボタン ${submit.height.toFixed(0)}px）`;
      } finally {
        await context.close();
      }
    });
  ...
  // ---- 3. スキップしても進行が止まらない ----
  {
    await runCase('スキップしても記録と進行が保たれる', async () => {
  ...
  {
    await runCase('会話の置換記号が残らない', async () => {
      ...
      try {
        ...
        return `（${linesSeen}行・観測した一人称: ${[...pronouns].join(' / ') || 'なし'}）`;
      } finally {
        await context.close();
      }
    });
