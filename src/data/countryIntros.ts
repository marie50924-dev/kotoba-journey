 ...
 * 遊ぶ人へ伝える学習情報なので、事実として本文確認の対象にする。
...
const JAPAN_SOURCES: InfoSource[] = [
  ...
  {
    // 場面別動画の公式な掲載元を示す案内ページ。
    // 事実文章の直接の根拠ではないため、sourceIds には入れていない。
    // 場面別 MP4 を確認するための入口・候補資料として残す。
    id: 'kankocho-manners',
  ...
  //
  // 人が公式 MP4 そのものを直接再生して内容を確かめたので body-checked。
  // どの場面を見て確認したのかが claim 側の sourceIds から追える。
  {
...
// 未確認の事実を作る補助（pending）は、日本の表示対象が
// あいさつ1件を残して本文確認済みになったため、いまは使い手がいない。
// 新しい国や文章を足すときに、また置くこと。
...
export const COUNTRY_INTROS: readonly CountryIntro[] = [
  {
    ...
    manners: [
      confirmed(
        ...
        ['kankocho-transport'],
        '観光庁公式ページから直接リンクされている公式MP4（04-30_En_1.mp4）を直接再生して確認した。降りる人を待って順番に乗ること、大きな荷物をほかの乗客のじゃまにならない場所へ置くこと、混雑した電車ではリュックを前に持つことが説明されていた。元の「大きな声で話さない」は動画内で確認できなかったため外し、確認できた荷物の話に直した。',
        '2026-09-21',
      ),
      confirmed(
        ...
        ['kankocho-temples'],
        '観光庁公式ページから直接リンクされている公式MP4（05-30_En_1.mp4）を直接再生して確認した。古い建物に触らないこと、祈っている参拝者のじゃまをしないことが説明されていた。元の「書かれている決まりを見てから入る」は確認できなかったため、動画で直接確認できた内容に直した。',
        '2026-09-21',
      ),
      confirmed(
        ...
        ['kankocho-baths'],
        '観光庁公式ページから直接リンクされている公式MP4（06-30_En_1.mp4）を直接再生して確認した。浴槽へ入る前に体を洗うよう説明されていた。元の「お風呂」は家庭の風呂まで含むように読めるため、資料が対象としている温泉・大浴場へ範囲を限定した。',
        '2026-09-21',
      ),
    ],
