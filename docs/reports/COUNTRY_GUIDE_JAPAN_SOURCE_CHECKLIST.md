...
確認対象16件のうち、**11件が本文確認済み**です。
残り5件が終わるまで日本の `publicationStatus` は `'draft'` のままで、
...
この16件が確認対象です。残り5件の本文を読んで判断してください。
...
| 本文確認状態 | **unchecked** |
| 本文中で確認した内容 | 公式PDF・CSV本体の直接確認待ち |
| 待っている確認 | 公式PDF・CSV本体の直接確認待ち。国土地理院「日本の主な山岳標高（1003山）」の PDF または CSV 本体で富士山の行を直接確認するまで body-checked にしない。 |
| 採用／修正／削除 | PDF・CSV本体の直接確認待ち |
| 確認日 | |
