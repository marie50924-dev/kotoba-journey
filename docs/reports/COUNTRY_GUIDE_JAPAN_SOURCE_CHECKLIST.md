...
子どもへ教える学習情報です。UI の飾りではないため、確認の対象にしています。
確認が終わるまで、画面には出しません。

### 46. `jp-claim-greeting-hello`
...
画面には `こんにちは` と `Hello` を出します。
上の文章は、その対応を1文にしたものです（表示する語から組み立てているので、
語を直せば文章も一緒に直ります）。claimId は文章を直しても変えません。
